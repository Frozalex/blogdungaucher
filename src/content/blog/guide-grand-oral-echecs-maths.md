...
excerpt: >-
  Tout ce dont tu as besoin pour ton Grand Oral spécialité Maths avec les échecs : méthodologie de l'épreuve, script
  minuté 10 min, formules prêtes, calculs détaillés, 25 questions de jury avec réponses rédigées. Imprime, surligne,
  performe.
...
seoDescription: >-
  Méthodologie, script complet minute par minute, formules Elo et loi binomiale prêtes, 25 questions de jury rédigées.
...
faq:
  - question: Quelle problématique choisir pour un Grand Oral Maths avec les échecs ?
    answer: >-
      ...
      probabilités permettent-elles de modéliser la performance aux échecs ?', idéal si tu maîtrises la loi binomiale
      et la formule Elo. (3) 'Pourquoi les mathématiques prouvent-elles que les échecs ne seront jamais résolus par un
  ...
  - question: Quelle est la durée idéale de chaque partie d'un exposé de Grand Oral Maths ?
    answer: >-
      ...
      minutage est critique : le jury interrompra au bout de 10 minutes. Prévois une alarme discrète à 9 minutes pour
      te signaler de conclure. Le jury pose ensuite ses questions pendant 10 minutes.
...
> **📥 Comment utiliser ce guide ?** Clique sur le bouton « Télécharger en PDF » en haut de la page pour récupérer le **guide** au format imprimable. Surligne en jaune les formules, en vert les phrases de transition, en rouge les **données** chiffrées. Entraîne-toi à dérouler chaque formule à voix haute sans regarder tes notes : le **jury** le remarque immédiatement.
...
|---|---|---|---|
| **Échecs et maths** *(notre choix)* | Combine combinatoire, probabilités, suites, complexité | Risque du récit pur sans formule | Faible avec ce **guide** |
...
> 
> $$P(\text{A bat B}) = \frac{1}{1 + 10^{(R_B - R_A)/400}}$$
> 
...
> 
> *A gagne contre B (résultat inattendu) :* $R'_A = 1600 + 16 \times (1 - 0{,}24) = 1612$
> 
> *A perd (résultat attendu) :* $R'_A = 1600 + 16 \times (0 - 0{,}24) = 1596$
> 
...
- **Partie 1 :** principe multiplicatif et nombre de Shannon (35^80 ≈ 10^124)
...
- **Partie 3 :** complexité O(b^d) → O(b^(d/2)) avec alpha-bêta, et **limites** matérielles
...
> *« Un tournoi à la ronde avec n joueurs donne n-1 parties à chaque joueur. Si je modélise chaque partie comme une expérience de Bernoulli de paramètre p, le score total d'un joueur suit B(n-1, p). Pour un tournoi de 10 joueurs (9 parties), avec p=0,5 : score moyen = 4,5, variance = 9×0,5×0,5 = 2,25, écart-type ≈ 1,5. En pratique, p varie selon l'adversaire : on utilise la somme de Bernoulli de paramètres différents. »*
...
║   Convergence : lim_(n→∞) u_n existe et est unique           ║
║   Loi des grands nombres : moyenne → espérance               ║
...
- **Silver, D., et al. (DeepMind, 2018).** [*A general reinforcement learning algorithm that masters chess, shogi, and Go.*](https://www.science.org/doi/10.1126/science.aar6404) *Science*, 362(6419). (AlphaZero, modèle d'apprentissage.)
