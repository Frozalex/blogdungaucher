---
title: "Mikhail Golubev — Le GM qui écrit comme il joue"
excerpt: "Golubev est un bon profil pour ton site: un joueur + un auteur, donc une passerelle entre pratique et explication."

firstName: "Mikhail"
lastName: "Golubev"
birthDate: "1970-05-30"
publishDate: "2026-05-30"
birthdayMonthDay: "05-30"
nationality: "Ukrainien"
era: "contemporain"
style: ["pragmatique", "positionnel", "ouvertures"]

angleHook: "Angle: l’utilité. Un auteur-GM permet de traiter une fiche comme un ‘outil’: quelles idées d’ouverture mènent à quels plans ?"
surprisingFact: "Il a été champion d’Ukraine en 1996."

iconicGame:
  title: "Une partie ‘ouverture → plan’"
  keyMoments:
    - "Préparation: choix d’une structure claire."
    - "Milieu: plan basé sur cases/structure."
    - "Finale: conversion en gardant l’activité."

tags: ["ukraine", "auteur", "ouvertures", "plans"]
seoTitle: "Mikhail Golubev : ouvertures, style, leçons | Blog d'un Gaucher"
seoDescription: "Portrait de Mikhail Golubev : faits essentiels et lecture de style prudente, orientée progression."
---

## Fiche (faits)

- **Né**: 30 mai 1970 (Odesa)  
- **Nationalité**: Ukraine  
- **Repère vérifiable**: GM (1996), champion d’Ukraine (1996)

## Lecture de style (analyse)

Dans un site “maillage interne”, Golubev est précieux pour relier des pages “ouvertures/structures” à une fiche joueur: la théorie n’est pas un musée, c’est une manière d’obtenir un type de milieu de jeu.

## Leçon pratique

- Choisis des ouvertures qui te donnent **des plans répétés**, pas seulement “des coups”.
