---
title: "Gennadi Sosonko - Le GM écrivain qui raconte l’intérieur"
excerpt: "Sosonko est une passerelle rare: un joueur d’élite qui sait aussi écrire sur le vécu des échecs."

firstName: "Gennadi"
lastName: "Sosonko"
birthDate: "1943-05-18"
publishDate: "2026-05-18"
birthdayMonthDay: "05-18"
nationality: "Néerlandais (né en URSS)"
era: "sovietique"
style: ["positionnel", "pragmatique", "universel"]

angleHook: "Angle: au lieu de ‘psychologiser’ sa personnalité, on s’appuie sur un fait solide: c’est un GM et un auteur. La fiche sert à relier jeu + narration."
surprisingFact: "Il a été champion des Pays-Bas à deux reprises."

iconicGame:
  title: "Une partie ‘mise sous contrôle’"
  keyMoments:
    - "Éviter les concessions structurelles."
    - "Améliorer les pièces avant de forcer."
    - "Finale où l’activité décide."

tags: ["pays-bas", "urss", "auteur", "writings"]
seoTitle: "Gennadi Sosonko : style, écriture, leçons | Blog d'un Gaucher"
seoDescription: "Portrait de Gennadi Sosonko : faits essentiels, angle ‘joueur + écrivain’ et leçons concrètes (sans invention)."
---

## Fiche (faits)

- **Né**: 18 mai 1943  
- **Nationalité**: URSS (avant 1972), Pays-Bas (après 1972)  
- **Repère vérifiable**: double champion des Pays-Bas, GM (1976)

## Pourquoi c’est une bonne fiche

Parce que son intérêt est double: la performance et la capacité à raconter le jeu de l’intérieur, sans le réduire à “des coups”.

## Leçon pratique

- Travaille ta capacité à **faire durer** une bonne position: c’est là que beaucoup craquent.
