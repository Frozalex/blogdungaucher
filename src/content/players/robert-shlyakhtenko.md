---
title: "Robert Shlyakhtenko - La progression ‘moderne’"
excerpt: "Un IM né en 2005: une bonne fiche pour parler de la nouvelle norme du haut niveau (travail, régularité, tournois)."

firstName: "Robert"
lastName: "Shlyakhtenko"
birthDate: "2005-05-09"
publishDate: "2026-05-09"
birthdayMonthDay: "05-09"
nationality: "Américain"
era: "contemporain"
style: ["tacticien", "pragmatique", "universel"]
photo:
  src: "https://upload.wikimedia.org/wikipedia/commons/c/cc/Empty_wooden_chessboard.jpg"
  alt: "Illustration - échiquier en bois (portrait non disponible)"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:Empty_wooden_chessboard.jpg"
  credit: "Paul Lloyd"
  license: "CC0 1.0"

angleHook: "Angle: une trajectoire contemporaine (IM très jeune) permet de parler de méthode: volume, tournois, régularité - sans inventer d’anecdotes."
surprisingFact: "Il a obtenu le titre de Maître International (IM) en 2022."

iconicGame:
  title: "Une partie ‘décisions propres’"
  keyMoments:
    - "Priorité à l’activité des pièces."
    - "Simplifications choisies."
    - "Conversion sans se précipiter."

tags: ["etats-unis", "jeune", "progression", "methodes"]
seoTitle: "Robert Shlyakhtenko : fiche IM, style, leçons | Blog d'un Gaucher"
seoDescription: "Portrait de Robert Shlyakhtenko : faits essentiels et lecture de style prudente, utile pour progresser."
---

## Fiche (faits)

- **Né**: 9 mai 2005 (Los Angeles, États-Unis)  
- **Titre**: International Master (2022)

## Lecture de style (analyse)

L’intérêt de ce type de profil, c’est moins “une légende” que la réalité du haut niveau moderne: gagner souvent, tenir longtemps, et transformer des petits avantages.

## Leçon pratique

- Cherche des positions où tu peux jouer **des coups simples et utiles** plusieurs fois d’affilée.
