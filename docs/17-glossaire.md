...
Un **fichier YAML** dans `.github/workflows/` qui définit une séquence de tâches automatiques déclenchées par des événements. Ce projet en a cinq : `scheduled-publish.yml`, `ntfy-notify.yml`, `newsletter.yml`, `check-publish-schedule.yml`, `secrets-scan.yml`. (Voir [chapitre 16](16-deploiement.md).)
