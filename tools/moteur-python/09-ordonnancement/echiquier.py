...
Article 4 de la série : ce fichier est identique à celui de l'article 3, et
c'est tout l'objet de l'article. `perft` ne modifie pas le générateur de coups,
il le juge.
...
class Coup:
    ...
    def __eq__(self, autre):
        # `NotImplemented` et non `False` : c'est ce qui permet de comparer un
        # coup à None sans exception. Les tables de killers de l'article 9 sont
        # initialisées à None, et le test `coup in killers[ply]` les traverse.
...
class Echiquier:
    ...
    def cle_position(self):
        """Identifiant d'une position pour la règle de la triple répétition.

        Deux positions sont « les mêmes » si l'échiquier, le trait, les droits
        de roque et les prises en passant disponibles coïncident. Les deux
        compteurs n'en font pas partie : une position est répétée même si le
        compteur des cinquante coups a avancé entre temps.
        """
        return (
    ...
    def materiel_insuffisant(self):
        """Aucun des deux camps ne peut mater, même avec la coopération de l'autre.

        On s'en tient aux cas que la FIDE traite en nulle immédiate : roi contre
        roi, roi et fou, roi et cavalier, et les deux fous de même couleur de
        case. Roi et deux cavaliers contre roi n'en fait pas partie : le mat y
        est possible, simplement pas forçable.
        """
        # `VIDE` valant None, on ne peut pas le tester avec `in "PpRrQq"` :
        # on filtre donc les cases vides d'abord.
        occupees = [case for case in CASES if self.cases[case] is not VIDE]
        if any(self.cases[case] in "PpRrQq" for case in occupees):
            return False
        ...
        if len(mineures) == 2 and all(self.cases[case] in "Bb" for case in mineures):
            # Deux fous : nulle seulement s'ils sont sur des cases de même
            # couleur, auquel cas aucun des deux ne contrôle l'autre teinte.
            couleurs = {(case // 10 + case % 10) % 2 for case in mineures}
            return len(couleurs) == 1
        return False

    def resultat(self, cles_vues=None):
        """Le verdict de la position, ou None si la partie continue.

        Renvoie un couple (résultat PGN, motif), le résultat valant "1-0",
        "0-1" ou "1/2-1/2". L'ordre des tests compte : le mat prime sur tout le
        reste, y compris sur la règle des cinquante coups.
        """
